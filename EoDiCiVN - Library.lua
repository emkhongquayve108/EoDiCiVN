...
local EoDiCiVNFolder = "EoDiCiVN"
local ConfigurationFolder = EoDiCiVNFolder.."/Configurations"
local ConfigurationExtension = ".rfld"



local EoDiCiVNLibrary = {
	Flags = {},
	Theme = {
		Default = {
			TextFont = "Default", -- Default will use the various font faces used across EoDiCiVN
...
if gethui then
	EoDiCiVN.Parent = gethui()
elseif syn.protect_gui then 
	syn.protect_gui(EoDiCiVN)
	EoDiCiVN.Parent = CoreGui
elseif CoreGui:FindFirstChild("RobloxGui") then
	EoDiCiVN.Parent = CoreGui:FindFirstChild("RobloxGui")
else
	EoDiCiVN.Parent = CoreGui
end
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == EoDiCiVN.Name and Interface ~= EoDiCiVN then
			Interface.Enabled = false
			Interface.Name = "EoDiCiVN-Old"
		end
	end
else
...
local Main = EoDiCiVN.Main
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList

EoDiCiVN.DisplayOrder = 100
...
local Notifications = EoDiCiVN.Notifications

local SelectedTheme = EoDiCiVNLibrary.Theme.Default

function ChangeTheme(ThemeName)
	SelectedTheme = EoDiCiVNLibrary.Theme[ThemeName]
	for _, obj in ipairs(EoDiCiVN:GetDescendants()) do
	...
	EoDiCiVN.Main.BackgroundColor3 = SelectedTheme.Background
	EoDiCiVN.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	EoDiCiVN.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	EoDiCiVN.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	EoDiCiVN.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	EoDiCiVN.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	EoDiCiVN.Main.Topbar.Theme.ImageColor3 = SelectedTheme.TextColor
...
local function LoadConfiguration(Configuration)
	...
	for FlagName, FlagValue in next, Data do
		if EoDiCiVNLibrary.Flags[FlagName] then
			spawn(function() 
				if EoDiCiVNLibrary.Flags[FlagName].Type == "ColorPicker" then
					EoDiCiVNLibrary.Flags[FlagName]:Set(UnpackColor(FlagValue))
				else
					if EoDiCiVNLibrary.Flags[FlagName].CurrentValue or EoDiCiVNLibrary.Flags[FlagName].CurrentKeybind or EoDiCiVNLibrary.Flags[FlagName].CurrentOption or EoDiCiVNLibrary.Flags[FlagName].Color ~= FlagValue then EoDiCiVNLibrary.Flags[FlagName]:Set(FlagValue) end
				end    
			end)
		else
			EoDiCiVNLibrary:Notify({Title = "Flag Error", Content = "EoDiCiVN was unable to find '"..FlagName.. "'' in the current script"})
		end
	end
end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i,v in pairs(EoDiCiVNLibrary.Flags) do
...
function EoDiCiVNLibrary:Notify(NotificationSettings)
	spawn(function()
		...
		if NotificationSettings.Actions then
			for _, Action in pairs(NotificationSettings.Actions) do
				...
				if SelectedTheme ~= EoDiCiVNLibrary.Theme.Default then
				...
				NewAction.MouseButton1Click:Connect(function()
					...
					if not Success then
						print("EoDiCiVN | Action: "..Action.Name.." Callback Error " ..tostring(Response))
					end
		...
		if EoDiCiVN.Name == "EoDiCiVN" then
...
function Hide()
	Debounce = true
	EoDiCiVNLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping K", Duration = 7})
...
function EoDiCiVNLibrary:CreateWindow(Settings)
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "EoDiCiVN Interface Suite"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "by Sirius"
	if Settings.LoadingTitle ~= "EoDiCiVN Interface Suite" then
		LoadingFrame.Version.Text = "EoDiCiVN UI"
	end
	...
	pcall(function()
		...
		if not isfolder(EoDiCiVNFolder.."/".."Configuration Folders") then
	...
	if Settings.Discord then
		if not isfolder(EoDiCiVNFolder.."/Discord Invites") then
			makefolder(EoDiCiVNFolder.."/Discord Invites")
		end
		if not isfile(EoDiCiVNFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(EoDiCiVNFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"EoDiCiVN RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		else
	...
	if Settings.KeySystem then
		...
		if not isfolder(EoDiCiVNFolder.."/Key System") then
			makefolder(EoDiCiVNFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("EoDiCiVN | "..Key.." Error " ..tostring(Response))
				end
			end
		end
		...
		if isfile(EoDiCiVNFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(EoDiCiVNFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(EoDiCiVNFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						EoDiCiVNLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully"})
					end
				else
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("EoDiCiVN | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					EoDiCiVNLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("EoDiCiVN | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Input.Title.Text = InputSettings.Name
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			for _, Option in ipairs(DropdownSettings.Options) do
				...
				DropdownOption.Interact.MouseButton1Click:Connect(function()
					...
					if not Success then
						...
						print("EoDiCiVN | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
						wait(0.5)
						Dropdown.Title.Text = DropdownSettings.Name
						TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					EoDiCiVNLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("EoDiCiVN | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					EoDiCiVNLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("EoDiCiVN | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
					TweenService:Create(Toggle.UIStroke, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {Transparency = 0}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
					EoDiCiVNLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if Dragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("EoDiCiVN | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Quint), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					EoDiCiVNLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
...
function EoDiCiVNLibrary:Destroy()
	EoDiCiVN:Destroy()
end
...
function EoDiCiVNLibrary:LoadConfiguration()
	if CEnabled then
		pcall(function()
			if isfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension) then
				LoadConfiguration(readfile(ConfigurationFolder .. "/" .. CFileName .. ConfigurationExtension))
				EoDiCiVNLibrary:Notify({Title = "Configuration Loaded", Content = "The configuration file for this script has been loaded from a previous session"})
			end
		end)
	end
end

task.delay(3.5, EoDiCiVNLibrary.LoadConfiguration, EoDiCiVNLibrary)
if EoDiCiVN:FindFirstChild("Notice") then
	EoDiCiVN.Notice.Visible = true
	EoDiCiVN.Notice.Interact.MouseButton1Click:Connect(function()
		EoDiCiVN.Notice.Visible = false
	end)
end

	return EoDiCiVNLibrary
